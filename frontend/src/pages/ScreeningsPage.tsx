...
export default function ScreeningsPage() {
  ...
  // Load jobs for the screening form
  const { data: jobs } = useQuery<JobPostingResponse[]>({
    queryKey: ['jobs'],
    queryFn: () => getJobs().then((r) => r.data),
  });

  // Load screening results for selected resume
  ...
  // Load all screening results for recruiter's organisation
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setScreenResumeId('');
      setScreenJobId('');
      // Refresh screenings if viewing the same resume
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-500 mt-1">Submit resumes for AI evaluation and view results</p>
      </div>

      {/* Submit for Screening */}
      {role === 'CANDIDATE' && (
        <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-xl p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Submit for Screening</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex items-end">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-medium transition-colors disabled:opacity-50 cursor-pointer"
              >
      ...
      {role === 'CANDIDATE' && resumes && resumes.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-xl p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-gray-900">View Results</h2>
            {selectedResumeId && (
              <button
          ...
          {selectedResumeId && screenings && screenings.length === 0 && (
            <div className="text-center py-10">
              ...
              <p className="text-gray-500">No screening results for this resume</p>
            </div>
          )}
      ...
      {role === 'CANDIDATE' && (!resumes || resumes.length === 0) && (
        <div className="bg-white border border-gray-200 rounded-xl p-10 text-center">
          ...
          <p className="text-gray-500">Upload a resume first to submit for screening</p>
        </div>
      )}
      ...
      {(role === 'RECRUITER' || role === 'ADMIN') && (
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-gray-900">Organisation Screening Results</h2>
          ...
          {!orgScreeningsLoading && (!orgScreenings || orgScreenings.length === 0) && (
            <div className="bg-white border border-gray-200 rounded-xl p-10 text-center">
              <BarChart3 className="w-10 h-10 text-gray-300 mx-auto mb-3" />
              <p className="text-gray-500">No screening results yet for your organisation</p>
            </div>
          )}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {(['COMPLETED', 'PROCESSING', 'PENDING', 'FAILED'] as const).map((status) => {
                  ...
                  const StatusIcon = cfg.icon;
                  return (
                    <div key={status} className={`rounded-xl p-4 ${cfg.bg} flex items-center gap-3`}>
                ...
                return (
                  <div key={s.id} className="bg-white border border-gray-200 rounded-xl p-6">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                      ...
                      {s.screeningStatus === 'COMPLETED' && s.matchScore !== null && (
                        <span className={`text-lg font-bold ${s.matchScore >= 75 ? 'text-green-600' : s.matchScore >= 50 ? 'text-amber-600' : 'text-red-600'}`}>
                          {s.matchScore}% match
                        </span>
                      )}
                    </div>
                    ...
                    {(s.screeningStatus === 'PENDING' || s.screeningStatus === 'PROCESSING') && (
                      <p className="text-sm text-gray-500">AI evaluation in progress...</p>
                    )}
                    {s.screeningStatus === 'FAILED' && (
                      <p className="text-sm text-red-600">Evaluation failed for this resume.</p>
                    )}
                  </div>
                );
              })}
